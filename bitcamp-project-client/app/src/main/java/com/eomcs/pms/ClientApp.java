...
public class ClientApp {
  ...
  public void execute() throws Exception {
    ...
    BoardDao boardDao = daoFactory.createDao(BoardDao.class); // daoFactory야 BoardDao 구현체 만들어줘~
  ...
  private Object createCommand(Class<?> clazz) throws Exception {
    // 생성자 정보를 알아낸다. 첫 번째 생성자만 꺼낸다. // 어차피 생성자가 하나밖에 없다.
    ...
    for (Parameter p : params) {
      ...
      args.add(findDependency(paramType)); // 있으면 저장. 없으면 null이라도 저장
    }
