...
//    - ~까지 인사 관리 업무가 어떻게 돌아가는지 프로그램을 실행시키면서 공부해라
@SuppressWarnings("serial")
@WebServlet("/error")
public class ErrorHandler extends HttpServlet {

  // 포워딩하는 측에서 클라이언트로부터 GET 요청을 받은 상태라면
  // 포워딩할 때 이 서블릿에 대해 GET 으로 요청할 것이다.
  // 포워딩하는 측에서 클라이언트로부터 POST 요청을 받은 상태라면
  // 포워딩할 때 이 서블릿에 대해 POST 로 요청할 것이다.
  // 결론!
  // - 따라서 이 서블릿은 GET 요청, POST 요청을 모두 처리할 수 있어야 한다.
  // 

  @Override // doGet 과 doPost 
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    if (e != null) {
      ...
      if (e.getMessage() != null) {
        out.printf("<p>%s</p>\n", e.getMessage()); // 오류 메시지가 있으면 출력. 없으면 실행 안함
      }
