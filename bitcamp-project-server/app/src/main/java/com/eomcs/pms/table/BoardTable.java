  ...
  public void service(Request request, Response response) throws Exception {
    ...
    switch (request.getCommand()) {
      case "board/insert":
        ...
        // 새 게시글의 번호
        // 게시글의 번호는 중간에 삭제가 일어난다고 해서 앞으로 당겨지면 안된다 -> 인덱스 사용하면 안됨
        // 삭제가 되면 번호 삭제되고 그냥 다음 번호!
        if (list.size() > 0) {
      ...
      case "board/selectall":
        for (Board b : list) {
          response.appendData(String.format("%d,%s,%s,%s,%d", // readUTF를 할 것이기 때문에 \n 필요없음
              b.getNo(), b.getTitle(), b.getWriter(), b.getRegisteredDate(), b.getViewCount()));
        }
        break;
      case "board/select":
        int no = Integer.parseInt(request.getData().get(0)); // 0번째 것 -> 번호
